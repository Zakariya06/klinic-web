...
export const useDoctorProfileStore = create<DoctorProfileState>((set, get) => ({
  ...
  setAvailableSpecializations: (specializations) =>
    set((state) => {
      ...
      if (
        state.availableSpecializations.length === 0 &&
  ...
  setAvailableQualifications: (qualifications) =>
    set((state) => {
      // Only set if empty (first time) and the input array is not empty
      if (
        state.availableQualifications.length === 0 &&
  ...
  updateClinic: (index, field, value) =>
    set((state) => ({
  ...
  prepareProfileData: () => {
    ...
    return {
      ...
      consultationFee: state.consultationFee
        ? parseInt(state.consultationFee)
        : undefined,
...
interface LaboratoryProfileState {
  ...
  addLaboratoryService: (
    service: Omit<LaboratoryService, "id" | "tests">,
  ) => void;
  ...
  prepareProfileData: () => any;
  reset: () => void;
  setServiceCoverImage: (serviceId: string, url: string) => void;
}

export const useLaboratoryProfileStore = create<LaboratoryProfileState>(
  (set, get) => ({
    // Initial state
    laboratoryName: "",
    laboratoryPhone: "",
    ...
    setLaboratoryGoogleMapsLink: (laboratoryGoogleMapsLink) =>
      set({ laboratoryGoogleMapsLink }),
    setCoverImage: (url) => set({ coverImage: url }),

    // ✅ NEW: update service cover image
    setServiceCoverImage: (serviceId: string, url: string) =>
      set((state) => ({
        laboratoryServices: state.laboratoryServices.map((service) =>
          service.id === serviceId ? { ...service, coverImage: url } : service,
        ),
      })),
    ...
    addAvailableSlot: (slot) =>
      set((state) => {
        if (state.availableSlots.includes(slot)) return state;
        return { availableSlots: [...state.availableSlots, slot].sort() };
      }),
    ...
    addLaboratoryService: (service) =>
      set((state) => {
        const tests = (service as any).tests
          ? (service as any).tests.map((test: any) => ({
              id:
    ...
    removeLaboratoryService: (serviceId) =>
      set((state) => ({
        laboratoryServices: state.laboratoryServices.filter(
    ...
    updateFromApiResponse: (data) => {
      ...
      set(() => {
        ...
        if (Array.isArray(data.laboratoryServices)) {
...
export const useProfileUIStore = create<ProfileUIState>((set) => ({
  ...
  setLabServiceCategories: (categories) =>
    set((state) => {
      // Only set categories if they haven't been loaded yet
      if (!state.hasLoadedLabServiceCategories && categories.length > 0) {
        return {
          labServiceCategories: categories,
          hasLoadedLabServiceCategories: true,
        };
      }
      return state;
    }),
  setHasLoadedAvailableCities: (loaded) =>
    set({ hasLoadedAvailableCities: loaded }),
  setHasLoadedAvailableSpecializations: (loaded) =>
    set({ hasLoadedAvailableSpecializations: loaded }),
  setHasLoadedAvailableQualifications: (loaded) =>
    set({ hasLoadedAvailableQualifications: loaded }),
  setHasLoadedLabServiceCategories: (loaded) =>
    set({ hasLoadedLabServiceCategories: loaded }),
  reset: () =>
    set({
      loading: false,
      uploading: false,
