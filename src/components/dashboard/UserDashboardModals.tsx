    ...
    >
      {prescriptionData?.appointments ? (
        prescriptionData.appointments.map((appointment: Appointment, index: number) => (
          <div key={appointment._id} className="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-200">
            <div className="flex items-center mb-3">
              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center mr-3">
                <FaUserMd size={16} className="text-blue-500" />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-gray-900">Dr. {appointment.providerName}</h3>
                <p className="text-sm text-gray-600">{formatAppointmentTime(appointment.timeSlot, appointment.timeSlotDisplay)}</p>
              </div>
            </div>
        ...
        <p className="text-gray-600 text-center">No prescription data available</p>
      )}
    </ModalWrapper>
  );
    ...
    >
      {allLabReports.length > 0 ? (
        allLabReports.map((report: Appointment, index: number) => (
          <div key={report._id} className="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-200">
            <div className="flex items-center mb-3">
              <div className="mr-3">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-gray-900">{report.providerName}</h3>
                <p className="text-sm text-gray-600">{report.serviceName}</p>
                <p className="text-xs text-gray-500">{formatAppointmentTime(report.timeSlot, report.timeSlotDisplay)}</p>
              </div>
            </div>
            ...
            {report.reportResult && (
              <div className="flex space-x-2">
                ...
                >
                  ...
                  <span className="text-purple-700 text-xs font-medium">Report</span>
                </button>
                ...
                >
                  ...
                  <span className="text-blue-700 text-xs font-medium">Notes</span>
                </button>
              </div>
            )}
          </div>
      ...
      {pdfsData && pdfsData.testReportPdfs && pdfsData.testReportPdfs.length > 0 ? (
        <div>
          <div className="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-200">
            <p className="text-sm font-medium text-gray-700 mb-2">Laboratory:</p>
            <p className="text-gray-900 text-sm">{pdfsData.labTest?.providerName}</p>
            <p className="text-sm font-medium text-gray-700 mb-2 mt-2">Service:</p>
            <p className="text-gray-900 text-sm">{pdfsData.labTest?.serviceName}</p>
          </div>
          ...
          {pdfsData.testReportPdfs.map((pdf: string, index: number) => (
            ...
            >
              ...
              <div className="flex-1">
                <p className="text-red-700 text-sm font-medium">Report Document {index + 1}</p>
        ...
        <div className="flex flex-col items-center py-8">
          ...
          <p className="text-gray-600 text-center">No PDF documents available</p>
        </div>
      )}
    </ModalWrapper>
  );
    ...
    >
      {notesData ? (
        <div>
          <div className="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-200">
            <p className="text-sm font-medium text-gray-700 mb-2">Laboratory:</p>
            <p className="text-gray-900 text-sm">{notesData.labTest?.providerName}</p>
            <p className="text-sm font-medium text-gray-700 mb-2 mt-2">Service:</p>
            <p className="text-gray-900 text-sm">{notesData.labTest?.serviceName}</p>
          </div>
          
          <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
            <p className="text-sm font-medium text-blue-700 mb-2">Laboratory Notes:</p>
  ...
  const LabReportDetailModal = () => (
    ...
    >
      {reportData ? (
        <div>
          <div className="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-200">
            <p className="text-sm font-medium text-gray-700 mb-2">Lab Report:</p>
            <p className="text-gray-900 text-sm leading-relaxed whitespace-pre-wrap">
              {reportData.report}
            </p>
          </div>
          ...
          {reportData.labTest?.testReportPdfs && reportData.labTest.testReportPdfs.length > 0 && (
            <div className="mt-4">
              <p className="text-sm font-medium text-gray-700 mb-2">Report Documents:</p>
              {reportData.labTest.testReportPdfs.map((pdf: string, index: number) => (
                ...
                >
                  <FaFileMedicalAlt size={16} className="text-red-500 mr-2" />
                  <p className="text-blue-700 text-sm font-medium flex-1">Report Document {index + 1}</p>
